...
export default function Hero(){return <section id="top" className={'hero hero-'+d.hero}>
 <div className="hero-copy"><p className="eyebrow">{d.tag}</p><h1>{d.title.map((s,i)=><span key={s} className={i?'title-second':''}>{s}</span>)}</h1><p className="hero-lead">{d.lead}</p><Actions/>
 <div className="hero-details"><div><small>Адрес</small><a href={d.maps.yandex} target="_blank" rel="noreferrer">{d.address}</a></div>{d.hours&&<div><small>Режим работы</small><strong>{d.hours}</strong></div>}</div>{d.note&&<p className="hero-note">{d.note}</p>}</div>
 <Photo index={0} hero className="hero-photo"/>
 {d.stamp&&<div className="hero-stamp"><strong>{d.stamp}</strong><span>{d.stampLabel}</span></div>}
 {d.hero==='waypoint'&&<a className="waypoint-card" href={phone}><small>Перед выездом</small><strong>Уточните въезд</strong><span>{d.business.phones[0]}<Arrow/></span></a>}
 ...
 {d.hero==='connect'&&<a className="connect-strip" href={phone}><span>Выездной шиномонтаж</span><strong>Уточнить адрес выезда →</strong></a>}
